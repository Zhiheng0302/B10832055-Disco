...
// 初始內容
function setup() {
  createCanvas(900, 700, WEBGL); // 決定 使用 3D 方式進行渲染
  ...
  //nb3 = new myBox(-150,100,0,50);
    ele = createAudio('Xue Hua Piao Piao Eurobeat Remix (wShinkoNet).mp3');
  ...
  let val = random(255);
  background(val);
  window.alert('滑鼠碰到disco球會增加速度');
  window.alert('滑鼠在畫面移動會改變背景線條顏色');
  window.alert('開始享受你的disco吧！');
}
...
class stela{
  ...
  display(){
    ...
    stroke(180,mouseX,255,mouseY);
    noFill(150,mouseY,mouseX);
    box(this.size);
    pop();
  }
}
